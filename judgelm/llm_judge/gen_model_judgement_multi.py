...
from judgelm.llm_judge.common import load_questions, reorg_answer_file, conv_judge_multi, conv_judge_multi_w_reference, KeywordsStoppingCriteria, parse_score, translate_score_to_win_list
...
def get_model_answers(
    ...
    for q_i, question in tqdm(enumerate(questions)):
        ...
        conv = conv_judge_multi.copy(answer_num) if references is None else conv_judge_multi_w_reference.copy(answer_num)
        ...
        answer_more = ""
        for i in range(answer_num - 2):
            answer_more += conv.answer_format.format(answer_id=i+3, answer=question[f'answer{i+3}_body'])
